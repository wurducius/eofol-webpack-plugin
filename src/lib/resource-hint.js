const resourceHint = (options) => {
  const preload = options.resourceHints.preload.map((item) => {
  ...
  const prefetch = options.resourceHints.prefetch.map((item) => {
    const isString = typeof item === "string"
    const href = isString ? item : item.url
    const as = isString ? false : item.as
  ...
  const preconnect = options.resourceHints.preconnect.map((item) => {
    const isString = typeof item === "string"
    const href = isString ? item : item.url
